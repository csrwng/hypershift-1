...
func ReconcileKonnectivityServerSecret(secret, ca *corev1.Secret, ownerRef config.OwnerRef) error {
	dnsNames := []string{
		...
		"konnectivity-server-proxy",
		fmt.Sprintf("konnectivity-server-proxy.%s.svc", secret.Namespace),
		fmt.Sprintf("konnectivity-server-proxy.%s.svc.cluster.local", secret.Namespace),
	}
