...
func (r *ControlPlaneAutoscalerController) updateSizeRecommendation(ctx context.Context, vpa *vpaautoscalingv1.VerticalPodAutoscaler, hc *hyperv1.HostedCluster) error {
	...
	if recommendedSize == "" {
		// No recommendation available for kube-apiserver yet; nothing to update
		return nil
	}
	if hc.Annotations != nil && hc.Annotations[hyperv1.RecommendedClusterSizeAnnotation] == recommendedSize {
		// No update needed
		return nil
	}
