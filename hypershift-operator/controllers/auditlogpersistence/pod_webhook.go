...
func (h *PodWebhookHandler) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	if err := h.mutatePod(ctx, mutated, config); err != nil {
...
func (h *PodWebhookHandler) mutatePod(ctx context.Context, pod *corev1.Pod, config *auditlogpersistencev1alpha1.AuditLogPersistenceConfig) error {
		...
		Spec: corev1.PersistentVolumeClaimSpec{
			...
			Resources: corev1.VolumeResourceRequirements{
				Requests: corev1.ResourceList{
					corev1.ResourceStorage: config.Spec.Storage.Size,
				},
			},
		},
	}

	if config.Spec.Storage.StorageClassName != "" {
		pvc.Spec.StorageClassName = &config.Spec.Storage.StorageClassName
	}
