...
func (h *ConfigMapWebhookHandler) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	if err := h.mutateConfigMap(mutated, config); err != nil {
	...
	h.log.Info("Successfully mutated ConfigMap for audit log persistence", "configmap", configMap.Name, "namespace", configMap.Namespace, "maxSize", config.Spec.AuditLog.MaxSize, "maxBackup", config.Spec.AuditLog.MaxBackup)
	return admission.PatchResponseFromRaw(req.Object.Raw, mutatedRaw)
}

func (h *ConfigMapWebhookHandler) mutateConfigMap(configMap *corev1.ConfigMap, config *auditlogpersistencev1alpha1.AuditLogPersistenceConfig) error {
	...
	if config.Spec.AuditLog.MaxSize != "" {
		apiServerArgs["audit-log-maxsize"] = []interface{}{config.Spec.AuditLog.MaxSize}
	}

	if config.Spec.AuditLog.MaxBackup > 0 {
		maxBackupStr := fmt.Sprintf("%d", config.Spec.AuditLog.MaxBackup)
